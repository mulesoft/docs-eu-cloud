...
=== Static IP Addresses
There are two static IP addresses that must be allowlisted to access the `mule-manager.eu1.anypoint.mulesoft.com` hostname:

[%header,cols="2*a"]
.Static IP Addresses to Allowlist
|===
|Name |IP Address
...
[%header,cols="1*a"]
.Hostnames Required in Firewall Rules
|===
|Hostname
